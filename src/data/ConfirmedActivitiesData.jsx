import alysonJonasMorato from "../assets/images/participacoes/2024/Alyson-Jonas-Alves-Morato.webp";
import caioHenriqueSousa from "../assets/images/participacoes/2024/Caio-Henrique-Lopes-Sousa.webp";
import danielMarquesGuimaraes from "../assets/images/participacoes/2024/Daniel-Marques-Vasconcelos-Guimaraes.webp";
import dayzaMeiryBarretoSilva from "../assets/images/participacoes/2024/Dayza Meiry Barreto Silva.webp";
import eduardoPaulinoDantas from "../assets/images/participacoes/2024/eduardo.webp";
import ellenSantosRamalho from "../assets/images/participacoes/2024/Ellen Santos Ramalho.webp";
import emillySantosRamalho from "../assets/images/participacoes/2024/Emilly Santos Ramalho.webp";
import erosFranklinSoaresFelix from "../assets/images/participacoes/2024/Eros Franklin Soares Felix.webp";
import esdrasSamuelFerreira from "../assets/images/participacoes/2024/Esdras Samuel de Araújo Ferreira.webp";
import gabrielLiraNobrega from "../assets/images/participacoes/2024/Gabriel Lira da Nóbrega.webp";
import giordaniAlvesFormiga from "../assets/images/participacoes/2024/Giordanni Alves Formiga.webp";
import giovannaTrigueiroAlmeida from "../assets/images/participacoes/2024/Giovanna-Trigueiro-de-Almeida-Araújo.webp";
import halanCaioNascimento from "../assets/images/participacoes/2024/Halan-Caio-Nascimento.webp";
import italloMendesSilva from "../assets/images/participacoes/2024/Itallo Mendes da Silva.webp";
import JoseAldoSilvadaCosta from "../assets/images/participacoes/2024/José-Aldo-Silva-da-Costa.webp";
import joaoLucasSousaMartins from "../assets/images/participacoes/2024/João Lucas de Sousa Martins.webp";
import jucelioSoaresSantos from "../assets/images/participacoes/2024/Jucelio Soares dos Santos.webp";
import kallebDiasMarques from "../assets/images/participacoes/2024/Kalleb-Dias-Rodrigues-Marques.webp";
import klaudioHenriqueMascarenhasMedeiros from "../assets/images/participacoes/2024/Klaudio-Henrique-Mascarenhas-Medeiros.webp";
import lauraRafaellaFeitosaAlves from "../assets/images/participacoes/2024/Laura Rafaella Feitosa Alves de Oliveira.webp";
import luannyKellyLeitao from "../assets/images/participacoes/2024/Luanny Kelly de Almeida Leitao.webp";
import lucianoClaudioSilvaSegundo from "../assets/images/participacoes/2024/Luciano Claudio Alves da Silva Segundo.webp";
import pedroFerreiraLimaNeto from "../assets/images/participacoes/2024/Pedro Ferreira de Lima Neto.webp";
import rebecaBarbosaLourenco from "../assets/images/participacoes/2024/Rebeca Barbosa Lourenço.webp";
import rodrigoAlvesCosta from "../assets/images/participacoes/2024/Rodrigo Alves Costa.webp";
import rosangelaAraújoMedeiros from "../assets/images/participacoes/2024/Rosângela de Araújo Medeiros.webp";
import thiagoAlvesLaurentino from "../assets/images/participacoes/2024/Thiago Alves Laurentino.webp";
import victorMenesesPinho from "../assets/images/participacoes/2024/Victor Meneses Pinho.webp";
import wellyngtonDaNobregaTargino from "../assets/images/participacoes/2024/Wellyngton-da-Nobrega-Targino.webp";
import yuriSantosMonteiro from "../assets/images/participacoes/2024/Yuri dos Santos Monteiro.webp";
import avatarNotFound from "../assets/images/image-not-found.png"
import camiladelucena from '../assets/images/minicursos/CamiladeLucena.jpg'
import deiseDias from '../assets/images/minicursos/DeiseDias.jpg'
import deboraArantes from '../assets/images/minicursos/DeboraAbrantes.jpg'
import beatrizGomes from '../assets/images/minicursos/BeatrizGomes.jpg'
import dunfreyAragao from '../assets/images/minicursos/DunfreyAragao.jpg'
import andreAlmeida from '../assets/images/minicursos/AndreAlmeida.jpg'
import jozieleAlmeida from '../assets/images/minicursos/JozieleAlmeida.jpg'
import humbertoNunes from '../assets/images/minicursos/HumbertoNunes.jpg'

export const ConfirmedActivitiesData = {
  Oficina: [
    {
      speakers: [
        {
          speakerImage: ellenSantosRamalho,
          speakerName: "Ellen Santos Ramalho",
        },
        {
          speakerImage: emillySantosRamalho,
          speakerName: "Emilly Santos Ramalho",
        },
        {
          speakerImage: rebecaBarbosaLourenco,
          speakerName: "Rebeca Barbosa Lourenço",
        }
      ],
      summaryText:
        "Mostrar aos participantes a oportunidade de alavancar suas carreiras por meio do LinkedIn, fornecendo estratégias eficazes para aumentar sua visibilidade, expandir sua rede e explorar oportunidades profissionais na plataforma.",
      confirmedActivityDescription: (
        <>
          <p>
            <strong>Idicação de Público: </strong>Todos os públicos.
          </p>
          <p>
            <strong>Pré-Requisito: </strong>Não tem nenhum pré-requisito.
          </p>
          <p>
            <strong>Objetivo: </strong>Mostrar aos participantes a oportunidade
            de alavancar suas carreiras por meio do LinkedIn, fornecendo
            estratégias eficazes para aumentar sua visibilidade, expandir sua
            rede e explorar oportunidades profissionais na plataforma.
          </p>
          <p>
            <strong>Justificativa: </strong>O LinkedIn se tornou um ponto
            central no mundo profissional, funcionando como um verdadeiro cartão
            de visitas virtual. Ter um perfil otimizado e uma rede sólida de
            contatos pode fazer toda a diferença na busca por novas
            oportunidades de emprego, colaborações profissionais e até mesmo no
            desenvolvimento de uma marca pessoal forte e reconhecida.
          </p>
          <p>
            <strong>Recursos: </strong>Datashow, televisão, lápis, apagador.
          </p>
          <p>
            <strong>Instalação de Softwares: </strong>Navegador.
          </p>
        </>
      ),
      confirmedActivityTitle:
        "LinkedIn na Prática: Estratégias e Oportunidades para Impulsionar sua Carreira",
    },

    {
      speakers: [
        {
          speakerImage: victorMenesesPinho,
          speakerName: "Victor Meneses Pinho",
        },
      ],
      summaryText:
        "Introduzir o aluno a um universo mais profundo em Java e backend",
      confirmedActivityDescription: (
        <>
          <p>
            <strong>Idicação de Público: </strong>Estudantes que gostem de
            programação principalmente em Java, POO, etc.
          </p>
          <p>
            <strong>Pré-Requisito: </strong>É interessante um prévio
            conhecimento na linguagem Java.
          </p>
          <p>
            <strong>Objetivo: </strong>Introduzir o aluno a um universo mais
            profundo em Java e backend.
          </p>
          <p>
            <strong>Justificativa: </strong>Tecnologia usada cada vez mais no
            mercado de trabalho, muitos recursos a serem utilizados.
          </p>
          <p>
            <strong>Recursos: </strong>Notebook, laptop
          </p>
          <p>
            <strong>Instalação de Softwares: </strong>Intellij IDEA, Java JDK,
            Insomnia.
          </p>
        </>
      ),
      confirmedActivityTitle:
        "Explorando o Spring Boot: Criando uma API Java para Operações CRUD",
    },

    {
      speakers: [
        {
          speakerImage: wellyngtonDaNobregaTargino,
          speakerName: "Wellyngton da Nobrega Targino",
        },
      ],
      summaryText:
        "Ensinar aos alunos conceitos de tratamento de dados e plotagem de gráficos",
      confirmedActivityDescription: (
        <>
          <p>
            <strong>Idicação de Público: </strong>Alunos do p1-p4.
          </p>
          <p>
            <strong>Pré-Requisito: </strong>Conhecimento de python.
          </p>
          <p>
            <strong>Objetivo: </strong>Ensinar aos alunos conceitos de
            tratamento de dados e plotagem de gráficos.
          </p>
          <p>
            <strong>Justificativa: </strong>Visto a temática, essas bibliotecas
            tem uma base inicial para a área de dados.
          </p>
          <p>
            <strong>Recursos: </strong>Pandas e o Matplotlib.
          </p>
          <p>
            <strong>Instalação de Softwares: </strong>De preferência o jupyter
            notebook ou a extensão no vs code e o python.
          </p>
        </>
      ),
      confirmedActivityTitle:
        "Plotagem de Gráficos com Pandas e Matplotlib a partir de Arquivos CSV",
    },
    {
      speakers: [
        {
          speakerImage: rosangelaAraújoMedeiros,
          speakerName: "Rosângela de Araújo Medeiros",
        },
      ],
      summaryText:
        "Nesta oficina, os participantes serão guiados através das principais etapas do processo de elaboração de um Trabalho de Conclusão de Curso (TCC) com foco na aplicação de técnicas de Inteligência Artificial (IA).",
      confirmedActivityDescription: (
        <>
          <p>
            <strong>Descrição: </strong>Nesta oficina, os participantes serão
            guiados através das principais etapas do processo de elaboração de
            um Trabalho de Conclusão de Curso (TCC) com foco na aplicação de
            técnicas de Inteligência Artificial (IA).
          </p>
          <p>
            <strong>Pré-Requisito:</strong> Ter conhecimento básico de
            metodologia científica.
          </p>
          <p>
            <strong>Recursos:</strong> Notebooks carregados para 2h de oficina.
          </p>
          <p>
            <strong>Instalação de Softwares:</strong> Navegador.
          </p>
        </>
      ),
      confirmedActivityTitle:
        "Estratégias para Elaboração de TCC com Aplicação de Inteligência Artificial",
    },
    {
      speakers: [
        {
          speakerImage: lauraRafaellaFeitosaAlves,
          speakerName: "Laura Rafaella Feitosa Alves de Oliveira",
        },
      ],
      summaryText:
        "Nesta oficina, os participantes serão introduzidos aos princípios essenciais de Design de Interface do Usuário (UI) e Experiência do Usuário (UX), utilizando a ferramenta Figma. Através de exemplos práticos e exercícios guiados, os participantes aprenderão a criar designs de interface atraentes e funcionais, levando em consideração aspectos como usabilidade, acessibilidade e estética.",
      confirmedActivityDescription: (
        <>
          <p>
            <strong>Descrição:</strong> Nesta oficina, os participantes serão
            introduzidos aos princípios essenciais de Design de Interface do
            Usuário (UI) e Experiência do Usuário (UX), utilizando a ferramenta
            Figma. Através de exemplos práticos e exercícios guiados, os
            participantes aprenderão a criar designs de interface atraentes e
            funcionais, levando em consideração aspectos como usabilidade,
            acessibilidade e estética.
          </p>
          <p>
            <strong>Pré-Requisito:</strong> Conhecimento básico de design
            gráfico e/ou interesse em aprender sobre design de interfaces.
          </p>
          <p>
            <strong>Recursos:</strong> Computador.
          </p>
          <p>
            <strong>Instalação de Softwares:</strong> Ter acesso a um computador
            com conexão à internet para utilizar o Figma.
          </p>
        </>
      ),
      confirmedActivityTitle:
        "Princípios Essenciais de Design UI/UX no Figma",
    },
    {
      speakers: [
        {
          speakerImage: lucianoClaudioSilvaSegundo,
          speakerName: "Luciano Claudio Alves da Silva Segundo",
        },
      ],
      summaryText:
        "Os participantes aprenderão a utilizar as estruturas de dados da biblioteca Collections em Java para resolver problemas de manipulação de dados de forma eficiente e elegante. Através de exemplos práticos e exercícios, os participantes irão explorar diferentes tipos de coleções, como Listas, Conjuntos e Mapas, e aprenderão a aplicar operações comuns, como adição, remoção, busca e ordenação de elementos.",
      confirmedActivityDescription: (
        <>
          <p>
            <strong>Descrição:</strong> Os participantes aprenderão a utilizar
            as estruturas de dados da biblioteca Collections em Java para
            resolver problemas de manipulação de dados de forma eficiente e
            elegante. Através de exemplos práticos e exercícios, os
            participantes irão explorar diferentes tipos de coleções, como
            Listas, Conjuntos e Mapas, e aprenderão a aplicar operações comuns,
            como adição, remoção, busca e ordenação de elementos.
          </p>
          <p>
            <strong>Pré-Requisito:</strong> Conhecimento básico de programação
            em Java.
          </p>
          <p>
            <strong>Recursos:</strong> Notebooks carregados para 2h de oficina.
          </p>
          <p>
            <strong>Instalação de Softwares:</strong> VSCode com o java
            instalado na máquina.
          </p>
        </>
      ),
      confirmedActivityTitle:
        "Utilizando Collections em Java para Resolução de Problemas de Manipulação de Dados",
    },
    {
      speakers: [
        {
          speakerImage: erosFranklinSoaresFelix,
          speakerName: "Eros Franklin Soares Felix",
        },
      ],
      summaryText:
        "Os participantes serão introduzidos aos princípios e práticas fundamentais do desenvolvimento front-end para criação de experiências web atrativas e funcionais. Através de uma abordagem prática e orientada a projetos, os participantes irão explorar conceitos de HTML, CSS e JavaScript.",
      confirmedActivityDescription: (
        <>
          <p>
            <strong>Descrição:</strong> Os participantes serão introduzidos aos
            princípios e práticas fundamentais do desenvolvimento front-end para
            criação de experiências web atrativas e funcionais. Através de uma
            abordagem prática e orientada a projetos, os participantes irão
            explorar conceitos de HTML, CSS e JavaScript.
          </p>
          <p>
            <strong>Pré-Requisito:</strong> Conhecimento básico de HTML, CSS e
            JavaScript.
          </p>
          <p>
            <strong>Recursos:</strong> Notebooks carregados para 2h de oficina.
          </p>
          <p>
            <strong>Instalação de Softwares:</strong> VSCode, Ferramentas de
            Inspeção de Navegador e Figma.
          </p>
        </>
      ),
      confirmedActivityTitle:
        "Front-end: Criando Experiências Web Atraentes e Funcionais.",
    },
    {
      speakers: [
        {
          speakerImage: thiagoAlvesLaurentino,
          speakerName: "Thiago Alves Laurentino",
        },
      ],
      summaryText:
        "Os participantes aprenderão a desenvolver aplicativos que podem ser executados em diversos sistemas operacionais, incluindo Windows, Linux e macOS, utilizando as ferramentas e recursos fornecidos pelo .NET Core.",
      confirmedActivityDescription: (
        <>
          <p>
            <strong>Descrição:</strong> Os participantes aprenderão a
            desenvolver aplicativos que podem ser executados em diversos
            sistemas operacionais, incluindo Windows, Linux e macOS, utilizando
            as ferramentas e recursos fornecidos pelo .NET Core.
          </p>
          <p>
            <strong>Pré-Requisito:</strong> Conhecimento básico de programação
            em C#.
          </p>
          <p>
            <strong>Recursos:</strong> Notebooks carregados para 2h de oficina.
          </p>
          <p>
            <strong>Instalação de Softwares:</strong> VSCode e .NET Core SDK.
          </p>
        </>
      ),
      confirmedActivityTitle:
        "Desenvolvimento Multiplataforma com C# utilizando . NET Core.",
    },
  ],
  MinicursoManha: [
    {
      speakers: [
        {
          speakerImage: camiladelucena,
          speakerName: "Camila de Lucena",
        }
      ],
      summaryText:
        "O minicurso tem como objetivo introduzir os participantes às ferramentas Git e GitHub.",
      confirmedActivityDescription: (
        <>
          <p>Este minicurso prepara profissionais de tecnologia para entrevistas em inglês, abordando vocabulário técnico, estrutura de respostas, perguntas comuns e dicas para se comunicar com confiança.</p>
        </>
      ),
      confirmedActivityTitle: " English for Tech Interviews: Como se Sair Bem em Entrevistas em Inglês?",
    },
    {
      speakers: [
        {
          speakerImage: deiseDias,
          speakerName: "Deise Dias",
        },
      ],
      summaryText: "Introduzir a linguagem C# na Universidade.",
      confirmedActivityDescription: (
        <>
          <p>Este minicurso explora como manter a produtividade sem cair no burnout. Focando na psicologia do trabalho, os participantes aprenderão a equilibrar exigências profissionais com o bem-estar emocional, utilizando técnicas para gerenciar o estresse, estabelecer limites saudáveis e promover a autoconsciência.</p>
        </>
      ),
      confirmedActivityTitle: " Produtividade Sem Burnout: Equilibrando Entregas e Bem-Estar",
    },
    {
      speakers: [
        {
          speakerImage: avatarNotFound,
          speakerName: "Ohana Lucena",
        },
      ],
      summaryText:
        "Minicurso projetado para introduzir os participantes ao desenvolvimento de jogos 2D usando a plataforma Construct 2. Este curso é ideal para entusiastas de jogos que desejam aprender a criar seus próprios jogos sem a necessidade de codificação complexa.",
      confirmedActivityDescription: (
        <>
          <p>
          Este minicurso oferece orientações práticas para a elaboração de um currículo eficaz e um LinkedIn otimizado. O objetivo é proporcionar estratégias para apresentar habilidades e experiências de forma clara, objetiva e alinhada às exigências do mercado, facilitando a visibilidade para recrutadores e oportunidades profissionais.
          </p>
        </>
      ),
      confirmedActivityTitle: "Currículo e LinkedIn: como se destacar? ",
    },
    {
      speakers: [
        {
          speakerImage: beatrizGomes,
          speakerName: "Beatriz Gomes",
        },
      ],
      summaryText:
        "Ensinar criar uma aplicação no framework SpringBoot que utiliza Inteligencia Artificial.",
      confirmedActivityDescription: (
        <>
          <p>
          Este minicurso tem como objetivo formar os participantes com conhecimentos teóricos e práticos sobre o funcionamento das blockchains, desde seus fundamentos até a construção de uma estrutura básica. O minicurso busca desmistificar o tema, oferecendo uma experiência prática acessível para quem deseja entender como essa tecnologia funciona na vida real.
          </p>
        </>
      ),
      confirmedActivityTitle:
        "Bora construir uma blockchain? Do conceito à prática",
    },
    {
      speakers: [
        {
          speakerImage: dunfreyAragao,
          speakerName: "Dunfrey Aragão",
        },
      ],
      summaryText:
        "Minicurso projetado para fornecer aos participantes uma introdução prática e acessível à visualização de dados usando a biblioteca Vega-lite. Este curso é ideal para iniciantes e profissionais que desejam aprender a criar visualizações de dados informativas e envolventes de forma eficaz.",
      confirmedActivityDescription: (
        <>
          <p>
          Este minicurso apresenta os conceitos essenciais do aprendizado de máquina, incluindo tipos de algoritmos, etapas de um projeto de ML e aplicações práticas, com foco na introdução acessível para iniciantes.
          </p>
        </>
      ),
      confirmedActivityTitle:
        "Fundamentos de machine learning ",
    },
    {
      speakers: [
        {
          speakerImage: andreAlmeida,
          speakerName: "André Almeida",
        },
      ],
      summaryText:
        "Minicurso focado em fornecer aos participantes uma compreensão abrangente e prática do tratamento de exceções em aplicações Java. Este curso é projetado para desenvolvedores Java de todos os níveis de habilidade que desejam aprimorar suas habilidades no tratamento de erros e exceções em seus aplicativos.",
      confirmedActivityDescription: (
        <>
          <p>
          O minicurso oferece uma introdução prática ao universo do desenvolvimento mobile. Os participantes aprenderão os conceitos fundamentais da plataforma Android, incluindo criação de interfaces, navegação entre telas, manipulação de dados e boas práticas de usabilidade. Ideal para quem deseja dar os primeiros passos na construção de aplicativos funcionais e modernos para smartphones.
          </p>
        </>
      ),
      confirmedActivityTitle: "Desenvolvimento de aplicativos móveis com Android",
    },
    // {
    //   speakers: [
    //     {
    //       speakerImage: yuriSantosMonteiro,
    //       speakerName: "Yuri dos Santos Monteiro",
    //     },
    //   ],
    //   summaryText:
    //     "Proporcionar aos alunos uma compreensão fundamental e prática dos conceitos básicos de desenvolvimento web, abrangendo HTML, CSS e JavaScript, preparando-os para construir suas próprias aplicações web simples.",
    //   confirmedActivityDescription: (
    //     <>
    //       <p>
    //         <strong>Idicação de Público: </strong>Pessoas que queiram aprender
    //         programação web mas ainda não tem uma base.
    //       </p>
    //       <p>
    //         <strong>Pré-Requisito: </strong>Sem pré-requisitos.
    //       </p>
    //       <p>
    //         <strong>Objetivo: </strong>Proporcionar aos alunos uma compreensão
    //         fundamental e prática dos conceitos básicos de desenvolvimento web,
    //         abrangendo HTML, CSS e JavaScript, preparando-os para construir suas
    //         próprias aplicações web simples.
    //       </p>
    //       <p>
    //         <strong>Justificativa: </strong>Devido à alta demanda por
    //         programadores web no mercado atual, é crucial adquirir habilidades
    //         nessa área. No entanto, algumas universidades oferecem aulas de
    //         programação web apenas nos períodos finais do curso. Nesse contexto,
    //         cursos complementares, se torna uma estratégia valiosa para
    //         antecipar e fortalecer as habilidades necessárias para atender às
    //         demandas do mercado de trabalho.
    //       </p>
    //       <p>
    //         <strong>Recursos: </strong>Datashow e se possível o laboratório.
    //       </p>
    //       <p>
    //         <strong>Instalação de Softwares: </strong>VSCode para escrever
    //         códigos em HTML, CSS e JavaScript e Navegador web moderno.
    //       </p>
    //     </>
    //   ),
    //   confirmedActivityTitle:
    //     "Introdução à Programação Web: Guia Básico em HTML, CSS e JavaScript",
    // },
    // {
    //   speakers: [
    //     {
    //       speakerImage: giovannaTrigueiroAlmeida,
    //       speakerName: "Giovanna Trigueiro de Almeida Araújo",
    //     },
    //   ],
    //   summaryText:
    //     "Explore os fundamentos e a prática da implementação de contratos inteligentes utilizando a linguagem Solidity. Este minicurso oferece uma introdução abrangente ao desenvolvimento de contratos inteligentes, abordando desde conceitos básicos até exemplos práticos de implementação.",
    //   confirmedActivityDescription: (
    //     <>
    //       <p>
    //         <strong>Descrição: </strong>Explore os fundamentos e a prática da
    //         implementação de contratos inteligentes utilizando a linguagem
    //         Solidity. Este minicurso oferece uma introdução abrangente ao
    //         desenvolvimento de contratos inteligentes, abordando desde conceitos
    //         básicos até exemplos práticos de implementação.
    //       </p>
    //       <p>
    //         <strong>Pré-Requisito: </strong>Conhecimentos básicos de programação
    //         e familiaridade com conceitos de blockchain. Alguma experiência com
    //         linguagens de programação semelhantes, como JavaScript ou C++
    //       </p>
    //       <p>
    //         <strong>Objetivo: </strong>Proporcionar aos alunos uma compreensão
    //         fundamental e prática dos conceitos básicos de desenvolvimento web,
    //         abrangendo HTML, CSS e JavaScript, preparando-os para construir suas
    //         próprias aplicações web simples.
    //       </p>
    //       <p>
    //         <strong>Justificativa: </strong>Devido à alta demanda por
    //         programadores web no mercado atual, é crucial adquirir habilidades
    //         nessa área. No entanto, algumas universidades oferecem aulas de
    //         programação web apenas nos períodos finais do curso. Nesse contexto,
    //         cursos complementares, se torna uma estratégia valiosa para
    //         antecipar e fortalecer as habilidades necessárias para atender às
    //         demandas do mercado de trabalho.
    //       </p>
    //       <p>
    //         <strong>Recursos: </strong>Sim, notebooks carregados para 2h de
    //         minicurso.
    //       </p>
    //       <p>
    //         <strong>Instalação de Softwares: </strong>Solidity
    //       </p>
    //     </>
    //   ),
    //   confirmedActivityTitle:
    //     "Desenvolvimento de Contratos Inteligentes com Solidity",
    // },
  ],
  MinicursoNoite: [
    {
      speakers: [
        {
          speakerImage: deboraArantes,
          speakerName: "Débora Abrantes",
        }
      ],
      summaryText:
        "O minicurso tem como objetivo introduzir os participantes às ferramentas Git e GitHub.",
      confirmedActivityDescription: (
        <>
          <p>
          Aprenda estratégias para resolver conflitos no ambiente tech, com foco em comunicação assertiva e colaboração eficiente. O objetivo é lidar com desacordos de forma construtiva e manter um ambiente de trabalho harmonioso.
          </p>
        </>
      ),
      confirmedActivityTitle: " Inteligência Emocional e Gestão de Conflitos na Carreira Tech",
    },
    {
      speakers: [
        {
          speakerImage: jucelioSoaresSantos,
          speakerName: "Jucelio Santos",
        },
      ],
      summaryText: "Introduzir a linguagem C# na Universidade.",
      confirmedActivityDescription: (
        <>
          <p>
          Este minicurso ensina técnicas para estruturar e comunicar ideias de forma clara. Os participantes aprenderão a criar apresentações cativantes, combinando storytelling e estratégias de pitch para conquistar a atenção do público e potenciais investidores.
          </p>
        </>
      ),
      confirmedActivityTitle: " Pitch e Storytelling: Como Apresentar Projetos de Forma Impactante",
    },
    {
      speakers: [
        {
          speakerImage: jozieleAlmeida,
          speakerName: "Joziele Almeida",
        },
      ],
      summaryText:
        "Minicurso projetado para introduzir os participantes ao desenvolvimento de jogos 2D usando a plataforma Construct 2. Este curso é ideal para entusiastas de jogos que desejam aprender a criar seus próprios jogos sem a necessidade de codificação complexa.",
      confirmedActivityDescription: (
        <>
          <p>
          Este minicurso oferece técnicas práticas para aprimorar a oratória, ajudando profissionais de tecnologia a se comunicarem com clareza, impacto e confiança.
          </p>
        </>
      ),
      confirmedActivityTitle: " Comunicação Eficaz para Profissionais de Tecnologia",
    },
    {
      speakers: [
        {
          speakerImage: klaudioHenriqueMascarenhasMedeiros,
          speakerName: "Klaudio Medeiros",
        },
      ],
      summaryText:
        "Ensinar criar uma aplicação no framework SpringBoot que utiliza Inteligencia Artificial.",
      confirmedActivityDescription: (
        <>
          <p>
          Os jogos digitais tem crescido na academia e na indústria, sendo uma área de interesse para estudantes e profissionais da Ciência da Computação. O objetivo do encontro é apresentar, elaborar e discutir oportunidades envolvendo jogos digitais para cientistas da computação, principalmente nos contextos de ensino, pesquisa e mercado.
          </p>
        </>
      ),
      confirmedActivityTitle:
        "Oportunidades envolvendo jogos digitais para cientistas da computação",
    },
    {
      speakers: [
        {
          speakerImage: yuriSantosMonteiro,
          speakerName: "Yuri Monteiro",
        },
      ],
      summaryText:
        "Minicurso projetado para fornecer aos participantes uma introdução prática e acessível à visualização de dados usando a biblioteca Vega-lite. Este curso é ideal para iniciantes e profissionais que desejam aprender a criar visualizações de dados informativas e envolventes de forma eficaz.",
      confirmedActivityDescription: (
        <>
          <p>
          Este minicurso apresenta as principais ferramentas da atualidade: Next.js, TailwindCSS e ShadCN UI. Os participantes aprenderão a criar aplicações web rápidas, responsivas e com design elegante, explorando desde a estruturação de projetos com Next.js até a criação de interfaces com componentes acessíveis usando ShadCN e estilização com Tailwind. O foco será na construção prática de interfaces modernas com boas práticas de código e organização.
          </p>
        </>
      ),
      confirmedActivityTitle:
        " Desenvolvimento web avançado: frameworks modernos",
    },
    {
      speakers: [
        {
          speakerImage: humbertoNunes,
          speakerName: "Humberto Nunes",
        },
      ],
      summaryText:
        "Minicurso focado em fornecer aos participantes uma compreensão abrangente e prática do tratamento de exceções em aplicações Java. Este curso é projetado para desenvolvedores Java de todos os níveis de habilidade que desejam aprimorar suas habilidades no tratamento de erros e exceções em seus aplicativos.",
      confirmedActivityDescription: (
        <>
          <p>
          Aprenda a manipular dados com pandas, criar gráficos e treinar modelos simples com Python. Tudo de forma prática e acessível, ideal para iniciantes em Ciência de Dados!
          </p>
        </>
      ),
      confirmedActivityTitle: " Introdução à Ciência de Dados com Python e Pandas",
    },
    // {
    //   speakers: [
    //     {
    //       speakerImage: yuriSantosMonteiro,
    //       speakerName: "Yuri dos Santos Monteiro",
    //     },
    //   ],
    //   summaryText:
    //     "Proporcionar aos alunos uma compreensão fundamental e prática dos conceitos básicos de desenvolvimento web, abrangendo HTML, CSS e JavaScript, preparando-os para construir suas próprias aplicações web simples.",
    //   confirmedActivityDescription: (
    //     <>
    //       <p>
    //         <strong>Idicação de Público: </strong>Pessoas que queiram aprender
    //         programação web mas ainda não tem uma base.
    //       </p>
    //       <p>
    //         <strong>Pré-Requisito: </strong>Sem pré-requisitos.
    //       </p>
    //       <p>
    //         <strong>Objetivo: </strong>Proporcionar aos alunos uma compreensão
    //         fundamental e prática dos conceitos básicos de desenvolvimento web,
    //         abrangendo HTML, CSS e JavaScript, preparando-os para construir suas
    //         próprias aplicações web simples.
    //       </p>
    //       <p>
    //         <strong>Justificativa: </strong>Devido à alta demanda por
    //         programadores web no mercado atual, é crucial adquirir habilidades
    //         nessa área. No entanto, algumas universidades oferecem aulas de
    //         programação web apenas nos períodos finais do curso. Nesse contexto,
    //         cursos complementares, se torna uma estratégia valiosa para
    //         antecipar e fortalecer as habilidades necessárias para atender às
    //         demandas do mercado de trabalho.
    //       </p>
    //       <p>
    //         <strong>Recursos: </strong>Datashow e se possível o laboratório.
    //       </p>
    //       <p>
    //         <strong>Instalação de Softwares: </strong>VSCode para escrever
    //         códigos em HTML, CSS e JavaScript e Navegador web moderno.
    //       </p>
    //     </>
    //   ),
    //   confirmedActivityTitle:
    //     "Introdução à Programação Web: Guia Básico em HTML, CSS e JavaScript",
    // },
    // {
    //   speakers: [
    //     {
    //       speakerImage: giovannaTrigueiroAlmeida,
    //       speakerName: "Giovanna Trigueiro de Almeida Araújo",
    //     },
    //   ],
    //   summaryText:
    //     "Explore os fundamentos e a prática da implementação de contratos inteligentes utilizando a linguagem Solidity. Este minicurso oferece uma introdução abrangente ao desenvolvimento de contratos inteligentes, abordando desde conceitos básicos até exemplos práticos de implementação.",
    //   confirmedActivityDescription: (
    //     <>
    //       <p>
    //         <strong>Descrição: </strong>Explore os fundamentos e a prática da
    //         implementação de contratos inteligentes utilizando a linguagem
    //         Solidity. Este minicurso oferece uma introdução abrangente ao
    //         desenvolvimento de contratos inteligentes, abordando desde conceitos
    //         básicos até exemplos práticos de implementação.
    //       </p>
    //       <p>
    //         <strong>Pré-Requisito: </strong>Conhecimentos básicos de programação
    //         e familiaridade com conceitos de blockchain. Alguma experiência com
    //         linguagens de programação semelhantes, como JavaScript ou C++
    //       </p>
    //       <p>
    //         <strong>Objetivo: </strong>Proporcionar aos alunos uma compreensão
    //         fundamental e prática dos conceitos básicos de desenvolvimento web,
    //         abrangendo HTML, CSS e JavaScript, preparando-os para construir suas
    //         próprias aplicações web simples.
    //       </p>
    //       <p>
    //         <strong>Justificativa: </strong>Devido à alta demanda por
    //         programadores web no mercado atual, é crucial adquirir habilidades
    //         nessa área. No entanto, algumas universidades oferecem aulas de
    //         programação web apenas nos períodos finais do curso. Nesse contexto,
    //         cursos complementares, se torna uma estratégia valiosa para
    //         antecipar e fortalecer as habilidades necessárias para atender às
    //         demandas do mercado de trabalho.
    //       </p>
    //       <p>
    //         <strong>Recursos: </strong>Sim, notebooks carregados para 2h de
    //         minicurso.
    //       </p>
    //       <p>
    //         <strong>Instalação de Softwares: </strong>Solidity
    //       </p>
    //     </>
    //   ),
    //   confirmedActivityTitle:
    //     "Desenvolvimento de Contratos Inteligentes com Solidity",
    // },
  ],
  Workshop: [
    {
      speakers: [
        {
          speakerImage: dayzaMeiryBarretoSilva,
          speakerName: "Dayza Meiry Barreto Silva",
        },
      ],
      summaryText:
        "Visa oferecer uma experiência de aprendizado respaldado na inclusão e acessibilidade para os  interessados, agregando conhecimentos. Ele tem como objetivo introduzir os participantes aos conceitos básicos da Língua Brasileira de Sinais (Libras) e sinais da área de Computação.",
      confirmedActivityDescription: (
        <>
          <p>
            <strong>Descrição:</strong> Visa oferecer uma experiência de
            aprendizado respaldado na inclusão e acessibilidade para os
            interessados, agregando conhecimentos. Ele tem como objetivo
            introduzir os participantes aos conceitos básicos da Língua
            Brasileira de Sinais (Libras) e sinais da área de Computação..
          </p>
          <p>
            <strong>Pré-Requisito:</strong> Nenhum conhecimento prévio é
            necessário.
          </p>
          <p>
            <strong>Recursos:</strong> Nenhum recurso é necessário.
          </p>
          <p>
            <strong>Instalação de Softwares:</strong> Não é necessário.
          </p>
        </>
      ),
      confirmedActivityTitle:
        "Code em Libras: Aprendendo Computação com Sinais",
    },
    {
      speakers: [
        {
          speakerImage: rodrigoAlvesCosta,
          speakerName: "Rodrigo Alves Costa",
        },
      ],
      summaryText:
        "Esta é a oportunidade perfeita para aprender como o Docker pode revolucionar a maneira como você lida com aplicativos e ambientes de software. ",
      confirmedActivityDescription: (
        <>
          <p>
            <strong>Descrição:</strong> Esta é a oportunidade perfeita para
            aprender como o Docker pode revolucionar a maneira como você lida
            com aplicativos e ambientes de software. Se você já se perguntou
            como evitar aquelas instalações confusas de programas, ou como
            garantir que suas configurações não causem conflitos, o Docker é a
            resposta.
          </p>
          <p>
            <strong>Pré-Requisito:</strong> Conhecimento básico de
            desenvolvimento de software e familiaridade com conceitos de
            virtualização e contêineres, mas não é estritamente necessário.
          </p>
        </>
      ),
      confirmedActivityTitle:
        "Docker 101: nunca mais instale nenhum app ?",
    },
    {
      speakers: [
        {
          speakerImage: esdrasSamuelFerreira,
          speakerName: "Esdras Samuel de Araújo Ferreira",
        },
      ],
      summaryText:
        "Workshop projetado para oferecer aos participantes uma introdução detalhada e prática ao testRigor, uma ferramenta de automação de testes de software. Durante o workshop, os participantes serão guiados por meio dos recursos e funcionalidades do testRigor, aprendendo como criar e executar testes automatizados de forma eficiente.",
      confirmedActivityDescription: (
        <>
          <p>
            <strong>Descrição:</strong> Workshop projetado para oferecer aos
            participantes uma introdução detalhada e prática ao testRigor, uma
            ferramenta de automação de testes de software. Durante o workshop,
            os participantes serão guiados por meio dos recursos e
            funcionalidades do testRigor, aprendendo como criar e executar
            testes automatizados de forma eficiente.
          </p>
          <p>
            <strong>Pré-Requisito:</strong> Conhecimento básico de testes de
            software e familiaridade com conceitos de automação de testes.
          </p>
          <p>
            <strong>Recursos:</strong> Não é necessário.
          </p>
          <p>
            <strong>Instalação de Softwares:</strong> Não é necessário.
          </p>
        </>
      ),
      confirmedActivityTitle: "Explorando o testRigor: Um Guia Prático",
    },
    {
      speakers: [
        {
          speakerImage: joaoLucasSousaMartins,
          speakerName: "João Lucas de Sousa Martins",
        },
      ],
      summaryText:
        "Workshop focado em fornecer aos participantes uma compreensão abrangente e prática do framework Django para desenvolvimento web em Python.",
      confirmedActivityDescription: (
        <>
          <p>
            <strong>Descrição:</strong> Workshop focado em fornecer aos
            participantes uma compreensão abrangente e prática do framework
            Django para desenvolvimento web em Python.
          </p>
          <p>
            <strong>Pré-Requisito:</strong> Conhecimento básico de Python,
            familiaridade com desenvolvimento web.
          </p>
          <p>
            <strong>Recursos:</strong> Não é necessário.
          </p>
          <p>
            <strong>Instalação de Softwares:</strong> Python, Django, um
            ambiente virtual (opcional), um editor de texto ou IDE e um banco de
            dados relacional.
          </p>
        </>
      ),
      confirmedActivityTitle:
        "Django em Foco: Tecnologias, Arquitetura e Projeto Prático",
    },
    {
      speakers: [
        {
          speakerImage: kallebDiasMarques,
          speakerName: "Kalleb Dias Rodrigues Marques",
        },
        {
          speakerImage: alysonJonasMorato,
          speakerName: "Alyson Jonas Alves Morato",
        },
      ],
      summaryText:
        "O Workshop oferece uma introdução envolvente aos fundamentos da animação 2D utilizando uma das ferramentas mais populares do mercado, o Paint Tool SAI. Durante este workshop, os participantes serão guiados em uma jornada emocionante através dos princípios essenciais da animação.",
      confirmedActivityDescription: (
        <>
          <p>
            <strong>Descrição:</strong> O Workshop oferece uma introdução
            envolvente aos fundamentos da animação 2D utilizando uma das
            ferramentas mais populares do mercado, o Paint Tool SAI. Durante
            este workshop, os participantes serão guiados em uma jornada
            emocionante através dos princípios essenciais da animação.
          </p>
          <p>
            <strong>Pré-Requisito:</strong> Conhecimento básico de desenho
            digital, computador com Paint Tool SAI instalado, interesse em
            animação 2D e motivação para praticar.
          </p>
          <p>
            <strong>Recursos:</strong> Notebooks carregados para 3h de workshop.
          </p>
          <p>
            <strong>Instalação de Softwares:</strong> Paint Tool SAI.
          </p>
        </>
      ),
      confirmedActivityTitle:
        "Explorando os Princípios da Animação 2D com Paint Tool SAI",
    },
    {
      speakers: [
        {
          speakerImage: luannyKellyLeitao,
          speakerName: "Luanny Kelly de Almeida Leitao",
        },
        {
          speakerImage: italloMendesSilva,
          speakerName: "Itallo Mendes da Silva",
        },
        {
          speakerImage: pedroFerreiraLimaNeto,
          speakerName: "Pedro Ferreira de Lima Neto",
        },
      ],
      summaryText:
        "Workshop projetado para proporcionar aos participantes as habilidades necessárias para se destacar em competições de robótica utilizando a linguagem de programação Java. Durante o workshop, os participantes aprenderão os fundamentos da programação em Java e como aplicá-los para programar robôs destinados a competições de alto nível, como as competições da FTC (First Tech Challenge).",
      confirmedActivityDescription: (
        <>
          <p>
            <strong>Descrição: </strong>Workshop projetado para proporcionar aos
            participantes as habilidades necessárias para se destacar em
            competições de robótica utilizando a linguagem de programação Java.
            Durante o workshop, os participantes aprenderão os fundamentos da
            programação em Java e como aplicá-los para programar robôs
            destinados a competições de alto nível, como as competições da FTC
            (First Tech Challenge).
          </p>
          <p>
            <strong>Pré-Requisito: </strong>Conhecimento básico de programação,
            familiaridade com a linguagem Java, interesse em robótica e
            disposição para aprender.
          </p>
          <p>
            <strong>Recursos: </strong>Notebooks carregados para 3h de workshop
          </p>
          <p>
            <strong>Instalação de Softwares:</strong> Java Development Kit
            (JDK), um ambiente de desenvolvimento integrado (IDE) como Eclipse
            ou IntelliJ IDEA, software de simulação ou ambiente de
            desenvolvimento para robótica, controlador de robô e plataforma de
            hardware, e ferramentas de controle de versão como Git
          </p>
        </>
      ),
      confirmedActivityTitle:
        "Desenvolvimento de Competências para Competições de Robótica com Java",
    },
    {
      speakers: [
        {
          speakerImage: jucelioSoaresSantos,
          speakerName: "Jucelio Soares dos Santos ",
        },
      ],
      summaryText:
        "Descubra como transformar dados em decisões estratégicas com o Power BI! Nesse workshop, você será guiado em uma jornada prática, desde a importação de dados até a criação de dashboards interativos. Aprenda a dominar as ferramentas essenciais do Power BI e aplique técnicas avançadas de análise para impulsionar a inteligência empresarial e otimizar a tomada de decisões.",
      confirmedActivityDescription: (
        <>
          <p>
            <strong>Descrição: </strong>Descubra como transformar dados em
            decisões estratégicas com o Power BI! Nesse workshop, você será
            guiado em uma jornada prática, desde a importação de dados até a
            criação de dashboards interativos. Aprenda a dominar as ferramentas
            essenciais do Power BI e aplique técnicas avançadas de análise para
            impulsionar a inteligência empresarial e otimizar a tomada de
            decisões.
          </p>
          <p>
            <strong>Pré-Requisito: </strong>Conhecimento básico de análise de
            dados
          </p>
          <p>
            <strong>Recursos: </strong>Notebooks carregados para 3h de workshop
          </p>
          <p>
            <strong>Instalação de Softwares:</strong> Microsoft Power BI
          </p>
        </>
      ),
      confirmedActivityTitle:
        "Revolucione sua Empresa com o Power BI: Transformando Dados em Decisões Estratégicas",
    },
    {
      speakers: [
        {
          speakerImage: JoseAldoSilvadaCosta,
          speakerName: "José Aldo Silva da Costa",
        },
      ],
      summaryText:
        "Desde o esboço inicial até a prototipagem avançada no Figma, exploraremos métodos de concepção, técnicas de design e ferramentas modernas para criar experiências de usuário excepcionais. Neste Workshop você terá a oportunidade de criar protótipos de baixo e alto nível - desde rabiscos simples até designs detalhados.",
      confirmedActivityDescription: (
        <>
          <p>
            <strong>Descrição: </strong>Desde o esboço inicial até a
            prototipagem avançada no Figma, exploraremos métodos de concepção,
            técnicas de design e ferramentas modernas para criar experiências de
            usuário excepcionais. Neste Workshop você terá a oportunidade de
            criar protótipos de baixo e alto nível - desde rabiscos simples até
            designs detalhados.
          </p>
          <p>
            <strong>Pré-Requisito: </strong> Conhecimento básico de design de
            interfaces é recomendado, mas não obrigatório
          </p>
          <p>
            <strong>Recursos: </strong> notebooks carregados para 3h de workshop
          </p>
          <p>
            <strong>Instalação de Softwares:</strong> Figma
          </p>
        </>
      ),
      confirmedActivityTitle:
        "Do Esboço ao Figma: Criando Protótipos de Interface de Baixo e Alto Nível ",
    },
  ],
};
