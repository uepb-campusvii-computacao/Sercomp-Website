...
function EventItem({ title, time, location, participants, timeStr, eventIndex }) {
  return (
    <li>
      ...
      {participants && participants.map((participant, index) => (
        eventIndex % 2 === 0 ? 
          <div key={index} className={`${styles.blockInfoTimeline} ${eventIndex % 2 === 0 ? styles.blockInfoTimelineRight : ''}`}>
            <p>{participant.name}</p>
            <img src={participant.photo} alt={participant.name} />            
          </div>
        :
        <div key={index} className={styles.blockInfoTimeline}>
          <img src={participant.photo} alt={participant.name} />
          <p>{participant.name}</p>
        </div>
      ))} 
...
function Timeline({ timelines }) {
  return (
    <section className={`${styles.timelineMain} event-timelines`}>
      <h1 className={styles.heading}>Acompanhe nossa <strong>Programação</strong></h1>
      <div className={`container ${styles.tabs}`}>
        <div className={styles.tabsHead}>
          {timelines.map((timeline, index) => (
            <div key={index} className={`${styles.tabsToggle} ${index === 0 ? styles.isActive : ''}`}>
        ...
        {timelines.map((timeline, index) => {
          const eventsWithIndex = timeline.events.map((event, eventIndex) => ({ ...event, eventIndex }));
